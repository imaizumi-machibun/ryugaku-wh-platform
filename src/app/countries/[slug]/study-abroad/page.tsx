...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const guide = await resolveCountryPurposeGuide(country, 'study-abroad').catch(() => null);
  const filtered = Object.values(searchParams).some(Boolean);
  const purposeVisuals = getCountryPurposeVisuals(params.slug, 'study-abroad');
  return generatePageMetadata({
    title: guide?.title ?? `${country.nameJp}留学完全ガイド｜学校・費用・ビザ・生活`,
    description: guide?.introduction ?? `${country.nameJp}留学の種類、学校選び、入学条件、学費、ビザ、住居、現地生活、卒業後まで詳しく解説。掲載校も学校名と都市から絞り込めます。`,
